...
for i=1:NumBootstraps
    ...
    ChosenOverlap=Overlap(k); %#ok<*PFBNS>
    ChosenRJSD=RootJSD(k);
    OverlapVector=ChosenOverlap(:);
    RJSDVector=ChosenRJSD(:);
    
    OverlapVector(unique([find(isnan(ChosenOverlap(:))) find(isnan(ChosenRJSD(:)))]))=[];
    RJSDVector(unique([find(isnan(ChosenOverlap(:))) find(isnan(ChosenRJSD(:)))]))=[];
