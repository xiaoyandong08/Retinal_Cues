function Diagram_of_Fig3abc_for_group05
...
for t=1:maxstep
    ...
    if is_generate_video == 1
        ...
        hold on;
        if t<=trace_length
            scatter(pixel_x(ICN_index6,:),pixel_y(ICN_index6,:),40,'r.');
            scatter(pixel_x(ICN_index7,:),pixel_y(ICN_index7,:),40,'g.');
            scatter(pixel_x(ICN_index8,:),pixel_y(ICN_index8,:),40,'b.');
            scatter(pixel_x(rest_index,:),pixel_y(rest_index,:),40,'.','MarkerEdgeColor',hex2rgb('CCCCCC'));
        else
            scatter(pixel_x(ICN_index6,end-trace_length:end),pixel_y(ICN_index6,end-trace_length:end),40,'r.');
            scatter(pixel_x(ICN_index7,end-trace_length:end),pixel_y(ICN_index7,end-trace_length:end),40,'g.');
            scatter(pixel_x(ICN_index8,end-trace_length:end),pixel_y(ICN_index8,end-trace_length:end),40,'b.');
            scatter(pixel_x(rest_index,end-trace_length:end),pixel_y(rest_index,end-trace_length:end),40,'.','MarkerEdgeColor',hex2rgb('CCCCCC'));
        end
        hold off

        subplot('position',[0.735 0.63 0.25 0.3]);
        plot([1:t],order_ICN_6,'r-','LineWidth',2);hold on
        plot([1:t],order_ICN_7,'g-','LineWidth',2)
        plot([1:t],order_ICN_8,'b-','LineWidth',2)
        plot([1:t],order_rest,'-','Color',hex2rgb('A6A8AB'),'LineWidth',2)
        ylabel(sprintf('order of\nbirds'' velocity'));
        set(gca,'Xticklabel','')
        set(gca,'FontSize',14)
        hold off
        ...
        if t<=size(Frame_matrix,2)-1
            ...
            plot([1:t],retina_O_dist_ICN_6,'r-','LineWidth',2);hold on
            plot([1:t],retina_O_dist_ICN_7,'g-','LineWidth',2)
            plot([1:t],retina_O_dist_ICN_8,'b-','LineWidth',2)
            plot([1:t],retina_O_dist_rest,'-','Color',hex2rgb('A6A8AB'),'LineWidth',2)
            ylabel('$\left < R_{24,j}(t,\tau=1)\right >_j$','Interpreter','latex')
            xlabel('t (frame)')
            set(gca,'FontSize',14)
            hold off
        end
    ...
    if is_generate_snapshot == 1
        if t == 50
            % 画Neighbors, Projection on ego-15s Retina when t = 50, 鸟的投影图片
            figure;
            ...
            set(gca, 'xticklabel', strtrim(sprintf('%s\\newline%s\n', x_labelArray{:})),'yticklabel', strtrim(sprintf('%s\\newline%s\n', y_labelArray{:}))); % 设置刻度标签
            set(gca,'FontSize',14)
            title(sprintf('Ego=%d, step=%d, spatial=%.2f, corrThresh=%.2f',focal_idx,t,ind_ave_spatial_value(focal_idx),threshold_ICNcorr));
            % 画ICN678的轨迹
            hold on;
            scatter(pixel_x(ICN_index6,:),pixel_y(ICN_index6,:),40,'r.');
            scatter(pixel_x(ICN_index7,:),pixel_y(ICN_index7,:),40,'g.');
        ...
        if t == maxstep
            %画3个变量的时序图
            figure;
            set(gcf,'Position',[911   375   306   426])
            subplot(3,1,1)
            %subaxis(3,1,1,'SpacingVertical',0.03,'MarginLeft',.15,'MarginRight',.05);%subplot('position',[0.35 0.04 0.25 0.26]);
            plot([1:t],order_ICN_6,'r-','LineWidth',2);hold on
            plot([1:t],order_ICN_7,'g-','LineWidth',2)
            plot([1:t],order_ICN_8,'b-','LineWidth',2)
            plot([1:t],order_rest,'-','Color',hex2rgb('A6A8AB'),'LineWidth',2)
            ylabel('order of birds'' velocities');
            set(gca,'Xticklabel','')
            set(gca,'FontSize',14)
            hold off
            ...
            plot([2:t],retina_O_dist_ICN_6,'r-','LineWidth',2);hold on
            plot([2:t],retina_O_dist_ICN_7,'g-','LineWidth',2)
            plot([2:t],retina_O_dist_ICN_8,'b-','LineWidth',2)
            plot([2:t],retina_O_dist_rest,'-','Color',hex2rgb('A6A8AB'),'LineWidth',2)
            ylabel('R_{24,j}(t,\tau=1)')
            set(gca,'Xticklabel','')
            set(gca,'FontSize',14)
            hold off
